...
import { TaskConfigService, effectiveDefaultRow } from "@/modules/project-management/task-configuration/services/task-config-service";
...
export class TaskService {
    ...
    static async listDepartmentTasks(actor: ScopedActor, permissions: PermissionContext): Promise<DepartmentTaskList> {
        const [source, catalogs] = await Promise.all([
    ...
    static async createTask(
        ...
        return TaskService.readCreatedTask(actor, created, buildShaping(catalogs, permissions));
    }
    ...
    /**
     * Reads the row back after a create. Directus normally answers with the created item, but a
     * 204/empty answer is documented behaviour for this instance's mutations, so the fallback asks
     * for the actor's newest live task in the department.
     */
    private static async readCreatedTask(
        actor: ScopedActor,
        created: unknown,
        shaping: RowShaping,
    ): Promise<TaskClientRow> {
        ...
        const row =
            createdId === null
                ? await TaskService.readNewestOwnTask(actor)
                : await readItem<RawTaskRow>("pm_task", createdId);
        if (row === null) {
            throw new TaskServiceError("INTERNAL_FAIL", "The task was created but could not be read back");
        }
        return toClientRow({ row, assignees: [], attachments: [] }, shaping);
    }

    /** The actor's newest live task in their department — the create read-back of last resort. */
