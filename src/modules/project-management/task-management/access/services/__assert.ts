/**
 * Phase-A assertion harness for the access policy.
 *
 ...
 *
 * The predicates in `./access-policy` are the exact functions the grant service calls, so the
 * revive-or-insert decision, the live-only access state, the tolerant flag reader and the department
...
import { AccessError, UNGRANTED_STATE, DEFAULT_ALLOW_ALL_MEMBERS_GRANT, findExistingAccessRow, findExistingSettingRow, accessStateByUserId, isTrueFlag, liveAccessRows, mergeMemberAccessState, resolveAllowAllMembersGrant, toPhTimestamp, toPositiveInt } from "./access-policy.ts";
...
check(
    "liveAccessRows keeps a live row and drops a revoked one",
    JSON.stringify(liveAccessRows([GRANT_HEAD, GRANT_REVOKED]).map((row) => row.id)) === "[11]",
);
check(
    "liveAccessRows reads string-shaped and Buffer-shaped flags",
    JSON.stringify(liveAccessRows([GRANT_STRING, GRANT_BUFFER_LIVE, GRANT_BUFFER_REVOKED]).map((row) => row.id)) === "[13,14]",
);

// --- The revive-or-insert decision ---------------------------------------------------------------

check("revive decision: a SOFT-DELETED row is still found (that is the revive path)", findExistingAccessRow([GRANT_HEAD, GRANT_REVOKED], 178)?.id === 12);
check("revive decision: no row for this member means INSERT", findExistingAccessRow([GRANT_HEAD, GRANT_REVOKED], 179) === null);
check("revive decision: another member's row is never the target", findExistingAccessRow([GRANT_HEAD], 999) === null);
check("revive decision: a string-shaped user id still matches", findExistingAccessRow([GRANT_STRING], 262)?.id === 13);
check(
    "revive decision: duplicates resolve deterministically to the lowest id",
    findExistingAccessRow([GRANT_DUPLICATE_LATE, GRANT_DUPLICATE_EARLY], 177)?.id === 20,
);

// --- Active-access state --------------------------------------------------------------------------

const STATE = accessStateByUserId([GRANT_HEAD, GRANT_REVOKED, GRANT_STRING, GRANT_BUFFER_LIVE, GRANT_BUFFER_REVOKED, GRANT_OUTSIDER]);
...
check("state: an empty grant table yields no state", accessStateByUserId([]).size === 0);
...
const MERGED = mergeMemberAccessState(MEMBERS, [GRANT_HEAD, GRANT_REVOKED, GRANT_OUTSIDER]);
...
check("merge: member fields ride along with the access state", MERGED[1].user_fname === "Probe" && MERGED[1].user_lname === "Granted");
check("merge: an empty grant table leaves every member un-granted", mergeMemberAccessState(MEMBERS, []).every((member) => !member.is_granted));

// --- Coded error ---------------------------------------------------------------------------------

const refusal = new AccessError("NOT_FOUND", "No live member with that user id exists in your department");
check(
    "AccessError carries its code and a CODE-prefixed message",
