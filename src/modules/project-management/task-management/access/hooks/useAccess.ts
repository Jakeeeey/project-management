 ...
 * same eight flags the rest of the module uses, and `canGrant` on that payload gates the
 * grant/revoke controls while `canManageDepartmentSetting` gates the policy switch — so a non-head
 * is told `canManageDepartmentSetting: false` by the server and the switch is not rendered into the
 * DOM at all.
 *
 * The department's `allow_all_members_grant` policy arrives as the sibling `setting` field and is
 * changed through `PATCH { allow_all_members_grant }` on the same endpoint. The route's PATCH is
 * HEAD-ONLY: a member may grant while the policy is ON, but the server refuses the member's attempt
 * to change the policy itself.
 *
 * Mutation strategy is single and fixed: **refetch after mutate**. There is no optimistic patching
 * anywhere in this module, so the roster and the switch can never drift from the server's answer.
 * Grant and revoke both answer **200** (not 201): the route deliberately returns 200 for a grant
 * because the write revives an existing soft-deleted row just as often as it inserts one, so there
...
const ENDPOINT = "/api/project-management/task-management/access";
 ...
 * granted individually.
    ...
    /**
     * Changes the department's `allow_all_members_grant` policy; resets the roster and the switch
     * from the server on success. The server refuses this for a non-head regardless of the flag.
     */
    readonly setAllowAllMembersGrant: (enabled: boolean) => Promise<boolean>;
}
...
export function useAccess(): UseAccessResult {
    ...
    const setAllowAllMembersGrant = useCallback(
