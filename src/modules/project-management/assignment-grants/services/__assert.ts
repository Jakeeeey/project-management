 ...
 *   node --experimental-strip-types src/modules/project-management/assignment-grants/services/__assert.ts
 ...
 * revive-or-insert decision, the live-only grant state and the tolerant flag reader are asserted
 * without a database. Why the `.ts` extension and the directive: Node resolves a relative import only
...
import { GrantError, UNGRANTED_STATE, findExistingGrantRow, grantStateByUserId, isTrueFlag, liveGrantRows, mergeMemberGrantState, toPhTimestamp, toPositiveInt } from "./grant-policy.ts";
