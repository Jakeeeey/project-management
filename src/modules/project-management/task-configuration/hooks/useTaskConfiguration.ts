 ...
 *
 * One route, one shape: every call goes to `/api/project-management/task-configuration` with only
...
export function useTaskConfiguration(): UseTaskConfigurationResult {
    ...
    const fetchConfig = useCallback(async (showLoading: boolean): Promise<void> => {
        ...
        try {
            const res = await fetch("/api/project-management/task-configuration", { cache: "no-store" });
    ...
    const request = useCallback(
        async (method: HttpMethod, body?: unknown, action?: string): Promise<Record<string, unknown>> => {
            const url = action
                ? `/api/project-management/task-configuration?action=${encodeURIComponent(action)}`
                : "/api/project-management/task-configuration";
