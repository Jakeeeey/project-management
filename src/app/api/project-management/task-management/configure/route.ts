...
} from "@/modules/project-management/task-management/configure/types/task-config.schema";
import { TaskConfigError, TaskConfigService } from "@/modules/project-management/task-management/configure/services/task-config-service";
...
function failureResponse(scope: string, error: unknown): NextResponse {
    ...
    if (error instanceof TaskConfigError) {
        ...
        console.error(`[configure ${scope}] catalog operation failed:`, error);
        return NextResponse.json(
            { success: false, message: "The catalog operation could not be completed. Please try again later." },
            { status: 500 },
        );
    }
    console.error(`[configure ${scope}] unexpected error:`, error);
    return NextResponse.json(
